...
# 获取  支持的交易对
def get_symbols(long_polling=None, request):
...
# 创建并执行订单
def send_order(amount, source, symbol, _type, request, price=0):
    """
    :param amount: 
...
# 查询某个订单
def order_info(order_id, request):
...
# 查询某个订单的成交明细
def order_matchresults(order_id, request):
